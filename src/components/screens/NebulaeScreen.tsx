...
export function NebulaeScreen({ onNav, onPick, onSelect, selected, desktop }: NebulaeScreenProps) {
  ...
  // Compute actual word count per nebula
  const wordCounts = NEBULAE.map(b => words.filter(w => boxOf(w.accuracy) === b.n).length);
  ...
  const StarMap = ({ height }: { height: number | string }) => (
    <div style={{
      ...
      {NEBULAE.map((b, i) => {
        ...
        return (
          <div key={b.n} onClick={() => onSelect(b.n)} className="nx-clickable"
            ...
            {/* Star label: name + word count */}
            <div style={{ position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', marginTop: desktop ? 10 : 8, whiteSpace: 'nowrap', textAlign: 'center', fontFamily: 'var(--display)', pointerEvents: 'none' }}>
              <div style={{ fontSize: desktop ? 13 : 9, letterSpacing: '0.1em', color: on ? b.color : 'var(--ink-soft)', textShadow: on ? `0 0 8px ${b.color}` : 'none' }}>
                {b.name.toUpperCase()}
              </div>
              <div className="nx-overline" style={{ fontSize: desktop ? 9 : 7, marginTop: 1, color: on ? b.color : 'var(--ink-mute)' }}>
                {count}
              </div>
            </div>
          </div>
      ...
      {desktop && (
        <>
          <div style={{ position: 'absolute', left: 14, bottom: 14, display: 'flex', gap: 12, fontSize: 11, color: 'var(--ink-soft)', zIndex: 2 }}>
            <span><span style={{ color: 'var(--mag)' }}>◄</span> {t('WEAK')}</span>
            <span style={{ color: 'var(--cyan)' }}>{t('STRONG')} ►</span>
          </div>
          <div style={{ position: 'absolute', right: 14, top: 14, zIndex: 2 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <NxCard glow="mag" bracket="mag" style={{ padding: 18 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                <NxTag>{selWordCount} WORDS</NxTag>
              </div>
            ...
            <NxCard style={{ padding: 14, flex: 1, overflow: 'hidden' }}>
              ...
              <div style={{ marginTop: 8 }}>
                {wordsInSel.length === 0 ? (
                  <div className="nx-overline" style={{ color: 'var(--ink-mute)', textAlign: 'center', marginTop: 12 }}>
                    // このゾーンに単語はありません
                  </div>
                ) : wordsInSel.map((w, i) => (
